...
from PySide6.QtWidgets import (
    ...
    QCheckBox, QLineEdit, QListWidget, QListWidgetItem, QInputDialog
)
...
from typing import Dict, Any, Optional, Tuple, List, Union # Union added
import os
import re
    ...
    try:
        ...
        class PanelInterface(QWidget): # type: ignore
            ...
            class ErrorLoggerMock:
                def log_error(self, msg, context=""): print(f"ERROR [{context or PanelInterface.PANEL_TYPE_NAME}]: {msg}")
                def log_warning(self, msg, context=""): print(f"WARNING [{context or PanelInterface.PANEL_TYPE_NAME}]: {msg}")
                def log_info(self, msg, context=""): print(f"INFO [{context or PanelInterface.PANEL_TYPE_NAME}]: {msg}")
                def log_debug(self, msg, context=""): print(f"DEBUG [{context or PanelInterface.PANEL_TYPE_NAME}]: {msg}")
...
class AdvancedPIDGeneratorWidget(PanelInterface):
    ...
    PANEL_DISPLAY_NAME: str = "高级PID代码生成器 v5 (多实例)"
    ...
    def __init__(self,
        ...
        # *** 初始化全局 code_config ***
        self.code_config: Dict[str, Any] = self._get_default_code_config() # <--- 在这里初始化
        ...
        self._init_ui_element_references() # 初始化所有可能的UI元素引用
        self._init_ui() # 构建UI, 会连接信号

        if initial_config:
            self.apply_config(initial_config) # apply_config 会处理 self.code_config 的加载
        else:
    ...
    def _init_ui(self) -> None:
        ...
        instance_management_group = QGroupBox("PID 控制器实例管理")
        ...
        list_area_layout.addWidget(self.pid_instance_list_widget)
        name_input_layout = QHBoxLayout()
        name_input_layout.addWidget(QLabel("实例名:"))
        self.instance_name_edit = QLineEdit()
        self.instance_name_edit.setPlaceholderText("例如: motor_pid, speed_pid")
        name_input_layout.addWidget(self.instance_name_edit)
        list_area_layout.addLayout(name_input_layout)
        instance_layout.addLayout(list_area_layout, 2) 
        instance_buttons_layout = QVBoxLayout()
        self.add_instance_button = QPushButton("添加实例")
        self.add_instance_button.clicked.connect(self._on_add_instance_button_clicked)
        self.remove_instance_button = QPushButton("移除选中")
        self.remove_instance_button.clicked.connect(self._on_remove_instance_button_clicked)
        self.rename_instance_button = QPushButton("重命名选中")
        ...
        instance_layout.addLayout(instance_buttons_layout, 1) 
        instance_management_group.setLayout(instance_layout)
        main_layout.addWidget(instance_management_group)
    ...
    def _create_basic_params_tab(self) -> QWidget: 
        ...
        core_layout.addWidget(QLabel("PID算法:"), 0, 0); self.algorithm_combo = QComboBox(); self.algorithm_combo.addItems(["高级PID", "位置式PID (内置)", "增量式PID (内置)"]); self.algorithm_combo.currentTextChanged.connect(self._on_pid_param_ui_changed); core_layout.addWidget(self.algorithm_combo, 0, 1)
        core_layout.addWidget(QLabel("PID类型 (P,D作用):"), 1, 0); self.pid_type_combo = QComboBox(); self.pid_type_combo.addItems(["标准 (Perr, Derr)", "PI-D (Perr, Dmeas)", "I-PD (Pmeas, Dmeas)"]); self.pid_type_combo.currentTextChanged.connect(self._on_pid_param_ui_changed); core_layout.addWidget(self.pid_type_combo, 1, 1)
        core_layout.addWidget(QLabel("工作模式 (输出):"), 2, 0); self.work_mode_combo = QComboBox(); self.work_mode_combo.addItems(["位置模式", "速度模式 (增量)"]); self.work_mode_combo.currentTextChanged.connect(self._on_pid_param_ui_changed); core_layout.addWidget(self.work_mode_combo, 2, 1)
        core_layout.addWidget(QLabel("比例 (Kp):"), 0, 2); self.kp_spinbox = QDoubleSpinBox(); self.kp_spinbox.setRange(-1000.0, 1000.0); self.kp_spinbox.setDecimals(6); self.kp_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); core_layout.addWidget(self.kp_spinbox, 0, 3)
        core_layout.addWidget(QLabel("积分 (Ki, 连续域):"), 1, 2); self.ki_spinbox = QDoubleSpinBox(); self.ki_spinbox.setRange(0, 1000.0); self.ki_spinbox.setDecimals(6); self.ki_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); core_layout.addWidget(self.ki_spinbox, 1, 3)
        core_layout.addWidget(QLabel("微分 (Kd, 连续域):"), 2, 2); self.kd_spinbox = QDoubleSpinBox(); self.kd_spinbox.setRange(0, 1000.0); self.kd_spinbox.setDecimals(6); self.kd_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); core_layout.addWidget(self.kd_spinbox, 2, 3)
        core_layout.addWidget(QLabel("采样时间 (s):"), 3, 2); self.sample_time_spinbox = QDoubleSpinBox(); self.sample_time_spinbox.setRange(0.000001, 10.0); self.sample_time_spinbox.setDecimals(6); self.sample_time_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); core_layout.addWidget(self.sample_time_spinbox, 3, 3)
        main_tab_layout.addWidget(core_params_group)
        limits_group = QGroupBox("输出与积分限制")
        limits_layout = QGridLayout(limits_group)
        limits_layout.addWidget(QLabel("最大输出:"), 0, 0); self.max_output_spinbox = QDoubleSpinBox(); self.max_output_spinbox.setRange(0, 10000.0); self.max_output_spinbox.setDecimals(2); self.max_output_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); limits_layout.addWidget(self.max_output_spinbox, 0, 1)
        limits_layout.addWidget(QLabel("最小输出:"), 0, 2); self.min_output_spinbox = QDoubleSpinBox(); self.min_output_spinbox.setRange(-10000.0, 0); self.min_output_spinbox.setDecimals(2); self.min_output_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); limits_layout.addWidget(self.min_output_spinbox, 0, 3)
        limits_layout.addWidget(QLabel("积分限幅 (绝对值):"), 1, 0); self.integral_limit_spinbox = QDoubleSpinBox(); self.integral_limit_spinbox.setRange(0.0, 10000.0); self.integral_limit_spinbox.setDecimals(2); self.integral_limit_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); limits_layout.addWidget(self.integral_limit_spinbox, 1, 1)
        limits_layout.addWidget(QLabel("输出变化率限制 (单位/秒, 0不限制):"), 1, 2); self.output_ramp_spinbox = QDoubleSpinBox(); self.output_ramp_spinbox.setRange(0.0, 10000.0); self.output_ramp_spinbox.setDecimals(3); self.output_ramp_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); limits_layout.addWidget(self.output_ramp_spinbox, 1, 3)
        main_tab_layout.addWidget(limits_group)
        adv_filter_group = QGroupBox("高级控制与滤波器")
        adv_filter_layout = QGridLayout(adv_filter_group)
        adv_filter_layout.addWidget(QLabel("前馈 (Kff):"), 0, 0); self.kff_spinbox = QDoubleSpinBox(); self.kff_spinbox.setRange(-1000.0, 1000.0); self.kff_spinbox.setDecimals(6); self.kff_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adv_filter_layout.addWidget(self.kff_spinbox, 0, 1)
        adv_filter_layout.addWidget(QLabel("前馈权重 (0-1):"), 0, 2); self.ff_weight_spinbox = QDoubleSpinBox(); self.ff_weight_spinbox.setRange(0.0, 1.0); self.ff_weight_spinbox.setDecimals(3); self.ff_weight_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adv_filter_layout.addWidget(self.ff_weight_spinbox, 0, 3)
        adv_filter_layout.addWidget(QLabel("死区大小:"), 1, 0); self.deadband_spinbox = QDoubleSpinBox(); self.deadband_spinbox.setRange(0.0, 1000.0); self.deadband_spinbox.setDecimals(6); self.deadband_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adv_filter_layout.addWidget(self.deadband_spinbox, 1, 1)
        adv_filter_layout.addWidget(QLabel("积分分离阈值 (误差绝对值):"), 1, 2); self.integral_separation_spinbox = QDoubleSpinBox(); self.integral_separation_spinbox.setRange(0.0, 10000.0); self.integral_separation_spinbox.setDecimals(3); self.integral_separation_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adv_filter_layout.addWidget(self.integral_separation_spinbox, 1, 3)
        adv_filter_layout.addWidget(QLabel("微分滤波系数 (0-1, 0无):"), 2, 0); self.d_filter_spinbox = QDoubleSpinBox(); self.d_filter_spinbox.setRange(0.0, 1.0); self.d_filter_spinbox.setDecimals(3); self.d_filter_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adv_filter_layout.addWidget(self.d_filter_spinbox, 2, 1)
        adv_filter_layout.addWidget(QLabel("输入滤波系数 (0-1, 0无):"), 2, 2); self.input_filter_spinbox = QDoubleSpinBox(); self.input_filter_spinbox.setRange(0.0, 1.0); self.input_filter_spinbox.setDecimals(3); self.input_filter_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adv_filter_layout.addWidget(self.input_filter_spinbox, 2, 3)
        adv_filter_layout.addWidget(QLabel("设定值滤波系数 (0-1, 0无):"), 3, 0); self.setpoint_filter_spinbox = QDoubleSpinBox(); self.setpoint_filter_spinbox.setRange(0.0, 1.0); self.setpoint_filter_spinbox.setDecimals(3); self.setpoint_filter_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adv_filter_layout.addWidget(self.setpoint_filter_spinbox, 3, 1)
    ...
    def _create_advanced_features_tab(self) -> QWidget: 
        ...
        self.adaptive_enable_checkbox = QCheckBox("启用自适应控制"); self.adaptive_enable_checkbox.stateChanged.connect(self._on_pid_param_ui_changed); adaptive_layout.addWidget(self.adaptive_enable_checkbox, 0,0,1,4)
        adaptive_layout.addWidget(QLabel("Kp 最小值:"), 1, 0); self.adaptive_kp_min_spinbox = QDoubleSpinBox(); self.adaptive_kp_min_spinbox.setRange(0.001,100); self.adaptive_kp_min_spinbox.setDecimals(6); self.adaptive_kp_min_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adaptive_layout.addWidget(self.adaptive_kp_min_spinbox,1,1)
        adaptive_layout.addWidget(QLabel("Kp 最大值:"), 1, 2); self.adaptive_kp_max_spinbox = QDoubleSpinBox(); self.adaptive_kp_max_spinbox.setRange(0.001,100); self.adaptive_kp_max_spinbox.setDecimals(6); self.adaptive_kp_max_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adaptive_layout.addWidget(self.adaptive_kp_max_spinbox,1,3)
        adaptive_layout.addWidget(QLabel("Ki 最小值:"), 2, 0); self.adaptive_ki_min_spinbox = QDoubleSpinBox(); self.adaptive_ki_min_spinbox.setRange(0.001,10); self.adaptive_ki_min_spinbox.setDecimals(6); self.adaptive_ki_min_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adaptive_layout.addWidget(self.adaptive_ki_min_spinbox,2,1)
        adaptive_layout.addWidget(QLabel("Ki 最大值:"), 2, 2); self.adaptive_ki_max_spinbox = QDoubleSpinBox(); self.adaptive_ki_max_spinbox.setRange(0.001,10); self.adaptive_ki_max_spinbox.setDecimals(6); self.adaptive_ki_max_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adaptive_layout.addWidget(self.adaptive_ki_max_spinbox,2,3)
        adaptive_layout.addWidget(QLabel("Kd 最小值:"), 3, 0); self.adaptive_kd_min_spinbox = QDoubleSpinBox(); self.adaptive_kd_min_spinbox.setRange(0.001,1); self.adaptive_kd_min_spinbox.setDecimals(6); self.adaptive_kd_min_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adaptive_layout.addWidget(self.adaptive_kd_min_spinbox,3,1)
        adaptive_layout.addWidget(QLabel("Kd 最大值:"), 3, 2); self.adaptive_kd_max_spinbox = QDoubleSpinBox(); self.adaptive_kd_max_spinbox.setRange(0.001,1); self.adaptive_kd_max_spinbox.setDecimals(6); self.adaptive_kd_max_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); adaptive_layout.addWidget(self.adaptive_kd_max_spinbox,3,3)
        layout.addWidget(adaptive_group)
        fuzzy_group = QGroupBox("模糊PID控制 (概念性, C逻辑需自行实现)")
        fuzzy_layout = QGridLayout(fuzzy_group)
        self.fuzzy_enable_checkbox = QCheckBox("启用模糊PID控制"); self.fuzzy_enable_checkbox.stateChanged.connect(self._on_pid_param_ui_changed); fuzzy_layout.addWidget(self.fuzzy_enable_checkbox,0,0,1,2)
        fuzzy_layout.addWidget(QLabel("模糊误差范围:"), 1, 0); self.fuzzy_error_range_spinbox = QDoubleSpinBox(); self.fuzzy_error_range_spinbox.setRange(1,1000); self.fuzzy_error_range_spinbox.setDecimals(2); self.fuzzy_error_range_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); fuzzy_layout.addWidget(self.fuzzy_error_range_spinbox,1,1)
        fuzzy_layout.addWidget(QLabel("模糊误差变化率范围:"), 2, 0); self.fuzzy_derror_range_spinbox = QDoubleSpinBox(); self.fuzzy_derror_range_spinbox.setRange(1,100); self.fuzzy_derror_range_spinbox.setDecimals(2); self.fuzzy_derror_range_spinbox.valueChanged.connect(self._on_pid_param_ui_changed); fuzzy_layout.addWidget(self.fuzzy_derror_range_spinbox,2,1)
    ...
    def _create_code_config_tab(self) -> QWidget: 
        ...
        grid_layout.addWidget(QLabel("库结构体名称:"), 0, 0); self.struct_name_edit = QLineEdit(); self.struct_name_edit.textChanged.connect(self._on_global_code_config_changed); grid_layout.addWidget(self.struct_name_edit, 0, 1)
        grid_layout.addWidget(QLabel("库函数前缀:"), 1, 0); self.function_prefix_edit = QLineEdit(); self.function_prefix_edit.textChanged.connect(self._on_global_code_config_changed); grid_layout.addWidget(self.function_prefix_edit, 1, 1)
        grid_layout.addWidget(QLabel("库头文件名称 (.h):"), 2, 0); self.header_name_edit = QLineEdit(); self.header_name_edit.textChanged.connect(self._on_global_code_config_changed); grid_layout.addWidget(self.header_name_edit, 2, 1)
        grid_layout.addWidget(QLabel("优化级别 (概念性):"), 3, 0); self.optimization_combo = QComboBox(); self.optimization_combo.addItems(["基础", "标准", "高级"]); self.optimization_combo.currentTextChanged.connect(self._on_global_code_config_changed); grid_layout.addWidget(self.optimization_combo, 3, 1)
        self.use_float_checkbox = QCheckBox("使用 float 类型 (否则 double)"); self.use_float_checkbox.stateChanged.connect(self._on_global_code_config_changed); grid_layout.addWidget(self.use_float_checkbox, 4, 0)
        self.include_comments_checkbox = QCheckBox("包含注释 (内置生成器)"); self.include_comments_checkbox.stateChanged.connect(self._on_global_code_config_changed); grid_layout.addWidget(self.include_comments_checkbox, 4, 1)
        self.include_header_checkbox = QCheckBox("生成头文件内容"); self.include_header_checkbox.stateChanged.connect(self._on_global_code_config_changed); grid_layout.addWidget(self.include_header_checkbox, 5, 0)
        self.use_template_checkbox = QCheckBox("使用模板生成库代码"); self.use_template_checkbox.stateChanged.connect(self._on_global_code_config_changed); grid_layout.addWidget(self.use_template_checkbox, 5, 1)
    ...
    def _on_add_instance_button_clicked(self):
        ...
        if not name_to_add: # 如果输入框为空，则弹出对话框
        ...
        if ok_pressed and name_to_add:
            ...
            if self.instance_name_edit: self.instance_name_edit.clear() # 清空输入框
        elif ok_pressed and not name_to_add: # 用户在对话框中点击OK但未输入名称
    ...
    def _add_pid_instance(self, name: str):
        ...
        self._load_params_to_ui(self.active_pid_instance_index)


    @Slot()
    def _on_remove_instance_button_clicked(self):
        if not self.pid_instance_list_widget or self.active_pid_instance_index < 0:
            QMessageBox.information(self, "移除实例", "请先选择一个要移除的PID实例。")
            return
        
        current_item = self.pid_instance_list_widget.currentItem()
        ...
        if reply == QMessageBox.Yes:
            self._log_debug(f"Removing PID instance: {instance_name} at index {self.active_pid_instance_index}")
            
            # 从列表中移除时要小心索引变化
            row_to_remove = self.active_pid_instance_index
            del self.pid_instance_configs[row_to_remove]
            self.pid_instance_list_widget.takeItem(row_to_remove) 
            
            # 更新 active_pid_instance_index 并选择新的当前项
            if self.pid_instance_list_widget.count() > 0:
                new_index = min(row_to_remove, self.pid_instance_list_widget.count() - 1)
                if new_index < 0: new_index = 0 # 以防万一
                self.pid_instance_list_widget.setCurrentRow(new_index)
                # _on_selected_pid_instance_changed 会被 currentItemChanged 信号触发
            else:
    ...
    def _on_rename_instance_button_clicked(self):
        ...
        old_name = self.pid_instance_configs[self.active_pid_instance_index]['name']
        
        # 优先使用 self.instance_name_edit 中的文本作为新名称建议
        ...
        if ok and new_name and new_name != old_name:
            # 检查新名称是否与现有其他实例冲突 (排除当前实例自身)
            if any(inst['name'] == new_name for i, inst in enumerate(self.pid_instance_configs) if i != self.active_pid_instance_index):
                QMessageBox.warning(self, "名称冲突", f"名为 '{new_name}' 的PID实例已存在。")
                return
            self._log_debug(f"Renaming PID instance '{old_name}' to '{new_name}'")
            self.pid_instance_configs[self.active_pid_instance_index]['name'] = new_name
            current_item.setText(new_name)
            if self.instance_name_edit: self.instance_name_edit.clear() # 清空输入框
            self._update_title() # 更新面板标题以反映新名称
    ...
    def _on_selected_pid_instance_changed(self, current: Optional[QListWidgetItem], previous: Optional[QListWidgetItem]):
        if not self.pid_instance_list_widget or not current:
            self.active_pid_instance_index = -1
            self._load_params_to_ui(-1) 
        ...
        self._log_debug(f"Selected PID instance: {instance_name} at index {self.active_pid_instance_index}")
        if self.instance_name_edit: self.instance_name_edit.setText(instance_name) # 更新名称编辑框
        self._load_params_to_ui(self.active_pid_instance_index)
        self._update_title()
    ...
    def _load_params_to_ui(self, instance_index: int):
        ...
        # 禁用信号以避免在加载时触发更新
        # (或者在_on_pid_param_ui_changed的开头检查是否是程序性更改)
        # 简单起见，我们依赖于用户交互来触发保存
        
        if self.algorithm_combo: self.algorithm_combo.setCurrentText({"advanced": "高级PID", "positional": "位置式PID (内置)", "incremental": "增量式PID (内置)"}.get(params_to_load.get("algorithm_type"), "高级PID"))
        if self.pid_type_combo: self.pid_type_combo.setCurrentText({"standard": "标准 (Perr, Derr)", "pi_d": "PI-D (Perr, Dmeas)", "i_pd": "I-PD (Pmeas, Dmeas)"}.get(params_to_load.get("pid_type"), "标准 (Perr, Derr)"))
        if self.work_mode_combo: self.work_mode_combo.setCurrentText({"position": "位置模式", "velocity": "速度模式 (增量)"}.get(params_to_load.get("work_mode"), "位置模式"))
        ...
        for key, spinbox in param_spinbox_map.items():
            if spinbox: spinbox.setValue(params_to_load.get(key, self._get_default_pid_params().get(key,0.0))) # 使用默认字典的默认值

        if self.adaptive_enable_checkbox: self.adaptive_enable_checkbox.setChecked(params_to_load.get("adaptive_enable", False))
        ...
        for tab_index in range(self.param_tabs.count()):
            ...
            if tab_name == "PID 参数" or tab_name == "高级特性":
                self.param_tabs.widget(tab_index).setEnabled(enable_editing)
    ...
    def _load_global_code_config_to_ui(self):
        ...
        if self.struct_name_edit: self.struct_name_edit.setText(self.code_config.get("struct_name", "PID_HandleTypeDef"))
        ...
        if self.optimization_combo: self.optimization_combo.setCurrentText(self.code_config.get("optimization_level", "标准").capitalize())
        checkbox_map = {
    ...
    def _on_pid_param_ui_changed(self):
        ...
        if self.algorithm_combo: params["algorithm_type"] = {"高级PID": "advanced", "位置式PID (内置)": "positional", "增量式PID (内置)": "incremental"}.get(self.algorithm_combo.currentText(), "advanced")
        if self.pid_type_combo: params["pid_type"] = {"标准 (Perr, Derr)": "standard", "PI-D (Perr, Dmeas)": "pi_d", "I-PD (Pmeas, Dmeas)": "i_pd"}.get(self.pid_type_combo.currentText(), "standard")
        if self.work_mode_combo: params["work_mode"] = {"位置模式": "position", "速度模式 (增量)": "velocity"}.get(self.work_mode_combo.currentText(), "position")
        ...
        if self.integral_separation_spinbox: params["integral_separation_threshold"] = self.integral_separation_spinbox.value()
    ...
    def _on_global_code_config_changed(self):
        if self.struct_name_edit: self.code_config["struct_name"] = self.struct_name_edit.text().strip() or "PID_HandleTypeDef"
        if self.function_prefix_edit: self.code_config["function_prefix"] = self.function_prefix_edit.text().strip() or "PID"
        if self.header_name_edit: self.code_config["header_name"] = self.header_name_edit.text().strip() or "pid.h"
        if self.optimization_combo: self.code_config["optimization_level"] = self.optimization_combo.currentText().lower()
        if self.use_float_checkbox: self.code_config["use_float"] = self.use_float_checkbox.isChecked()
        if self.include_comments_checkbox: self.code_config["include_comments"] = self.include_comments_checkbox.isChecked()
    ...
    def _generate_c_code(self) -> str: 
        ...
        if not (c_template_path and c_template_path.exists()):
            self._log_warning(f"Library C template not found ({c_template_path}), falling back to built-in.", self.PANEL_TYPE_NAME)
            return self._generate_builtin_code(use_instance_params=False)
    ...
    def _generate_main_c_code_for_all_instances(self) -> str:
        ...
        try:
            ...
            for i, instance_config in enumerate(self.pid_instance_configs):
                ...
                initializations_block.append(f"    {self.code_config.get('function_prefix')}_SetOutputLimits(&{instance_name}, {params.get('max_output', 100.0)}{sfx});")
                initializations_block.append(f"    {self.code_config.get('function_prefix')}_SetIntegralLimits(&{instance_name}, {params.get('integral_limit', 50.0)}{sfx});")
                initializations_block.append(f"    printf(\"Initialized PID: {instance_name} with Kp={kp_val}, Ki_cont={ki_val}, Kd_cont={kd_val}, Ts={sample_t}\\n\");")
                if i == 0: 
                    ...
                    example_computations_block.append(f"        {data_t} {instance_name}_output = {self.code_config.get('function_prefix')}_Compute(&{instance_name}, {instance_name}_setpoint, {instance_name}_measurement);")
                    example_computations_block.append(f"        {instance_name}_measurement = simulate_system_response({instance_name}_measurement, {instance_name}_output, {sample_t}{sfx}); ")
                    example_computations_block.append(f"        printf(\"  {instance_name}: Step %d, SP=%.2f, PV=%.2f, Out=%.2f\\n\", j+1, (double){instance_name}_setpoint, (double){instance_name}_measurement, (double){instance_name}_output);")
                    example_computations_block.append(f"    }}")
                    example_computations_block.append(f"")
            processed_code = re.sub(re.escape('{{PID_INSTANCE_DECLARATIONS}}'), "\n".join(declarations_block), processed_code)
            processed_code = re.sub(re.escape('{{PID_INSTANCE_INITIALIZATIONS}}'), "\n".join(initializations_block), processed_code)
            processed_code = re.sub(re.escape('{{PID_EXAMPLE_COMPUTATIONS}}'), "\n".join(example_computations_block), processed_code)
            single_instance_params = self.pid_instance_configs[0]['params'] if self.pid_instance_configs else self._get_default_pid_params()
    ...
    def _get_template_path(self, algo_type_override: Optional[str] = None, extension: str = "c") -> Optional[Path]:
        ...
        if not algo_type_to_use:
            if self.active_pid_instance_index != -1 and self.pid_instance_configs:
                algo_type_to_use = self.pid_instance_configs[self.active_pid_instance_index]['params'].get("algorithm_type", "advanced")
            else:
        ...
        if template_file.exists():
            ...
            self._log_warning(f"Template file not found: {template_file} for algorithm {algo_type_to_use}", self.PANEL_TYPE_NAME)
            return None

    def _generate_from_template_file(self, template_path: Path, use_instance_params: bool = True, instance_index_for_params: int = -1) -> str:
        self._log_debug(f"Generating from template: {template_path}, use_instance_params: {use_instance_params}, instance_idx: {instance_index_for_params}")
        try:
    ...
    def _update_title(self) -> None:
        try:
            title_suffix = ""
            if self.active_pid_instance_index != -1 and self.active_pid_instance_index < len(self.pid_instance_configs):
                title_suffix = f" - 编辑: {self.pid_instance_configs[self.active_pid_instance_index]['name']}"
            elif self.pid_instance_list_widget and self.pid_instance_list_widget.currentItem():
                 title_suffix = f" - 编辑: {self.pid_instance_list_widget.currentItem().text()}"
    ...
    def _log_error(self, message: str, context: Optional[str] = None):
        ...
        else: print(f"ERROR [{effective_context}]: {message}")
    ...
    def _log_warning(self, message: str, context: Optional[str] = None):
        ...
        else: print(f"WARNING [{effective_context}]: {message}")
    ...
    def _log_info(self, message: str, context: Optional[str] = None):
        ...
        else: print(f"INFO [{effective_context}]: {message}")
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "active_pid_instance_index": self.active_pid_instance_index,
    ...
    def apply_config(self, config: Dict[str, Any]) -> None:
        ...
        self.active_pid_instance_index = config.get("active_pid_instance_index", -1)
        global_code_cfg = config.get("global_code_config")
        if global_code_cfg:
            self.code_config.update(global_code_cfg)
        else: # 如果旧配置没有 global_code_config，则使用默认值初始化
        ...
        if self.pid_instance_list_widget:
            ...
            if self.active_pid_instance_index != -1 and self.active_pid_instance_index < self.pid_instance_list_widget.count():
                self.pid_instance_list_widget.setCurrentRow(self.active_pid_instance_index)
                # _on_selected_pid_instance_changed 会被触发并调用 _load_params_to_ui
            elif self.pid_instance_list_widget.count() > 0: 
                self.pid_instance_list_widget.setCurrentRow(0)
            else: 
                 self._load_params_to_ui(-1) 
